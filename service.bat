...
echo      3. Check Status
echo.
echo   :: SETTINGS (forcibly locked)
echo      4. Game Filter         [!GameFilterStatus!] (locked)
echo      5. IPSet Filter        [!IPsetStatus!] (locked)
echo      6. Auto-Update Check   [!CheckUpdatesStatus!] (locked)
echo.
echo   :: UPDATES (forcibly locked)
echo      7. Update IPSet List   [disabled]
echo      8. Update Hosts File   [disabled]
echo      9. Check for Updates   [disabled]
echo.
echo   :: TOOLS
...
if "%menu_choice%"=="4" goto locked
if "%menu_choice%"=="5" goto locked
if "%menu_choice%"=="6" goto locked
if "%menu_choice%"=="7" goto sorry
if "%menu_choice%"=="8" goto sorry
...
:sorry
cls
echo Sorry, this setting doesn't work.
echo.
echo Press any key...
pause > nul
goto menu
...
: locked
cls
echo Sorry, this setting is not changeable.
echo.
echo Press any key...
pause > nul
goto menu
...
set "LOCAL_VERSION=1.0.1"
set "GITHUB_VERSION_URL=https://raw.githubusercontent.com/ВАШ_ЛОГИН/НАЗВАНИЕ_РЕПОЗИТОРИЯ/main/.service/version.txt"
set "GITHUB_DOWNLOAD_URL=https://github.com/ВАШ_ЛОГИН/НАЗВАНИЕ_РЕПОЗИТОРИЯ/releases/latest"
...
:: GAME SWITCH (принудительно включен)
:game_switch_status
set "GameFilterStatus=enabled"
set "GameFilter=1024-65535"
exit /b
...
:: CHECK UPDATES SWITCH (принудительно выключен)
:check_updates_switch_status
set "CheckUpdatesStatus=disabled"
exit /b


:: IPSET SWITCH (принудительно any)
:ipset_switch_status
set "IPsetStatus=any"
exit /b


:: IPSET UPDATE (ОТКЛЮЧЕНО)
:ipset_update
echo IPSet update is disabled (forced any mode)
pause
goto menu


:: HOSTS UPDATE (ОТКЛЮЧЕНО)
:hosts_update
echo Hosts update is disabled
pause
goto menu
...
:locked_feature
echo This feature is disabled in this build
pause
goto menu
